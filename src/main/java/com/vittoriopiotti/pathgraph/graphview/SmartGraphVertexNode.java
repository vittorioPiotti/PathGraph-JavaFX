...
public class SmartGraphVertexNode<T> extends Group implements SmartGraphVertex<T>, SmartLabelledNode {
    ...
    */
    ...
    private double dragThresholdNode = 5.0; // Definisce la soglia per considerare un movimento come drag
    private double nodeStartX;
    private double nodeStartY;
    private Runnable hideContextMenu;

    private BiConsumer<MouseEvent, Vertex<T>> onClickNode;
     ...
     */
    public SmartGraphVertexNode(Vertex<T> v, double x, double y, double radius, String shapeType, boolean allowMove, BiConsumer<MouseEvent, Vertex<T>>  onClickNode,Runnable hideContextMenu) {
    ...
    /**
     * Sets the callbacks for hiding the context menu and handling node click events.
     *
     * @param hideContextMenu Runnable to hide the context menu.
     * @param onClickNode BiConsumer for handling node clicks with a MouseEvent and Vertex.
     * @author vittoriopiotti
     */
    public void setAllCallbacks(Runnable hideContextMenu, BiConsumer<MouseEvent, Vertex<T>> onClickNode){
        this.hideContextMenu = hideContextMenu;
        this.onClickNode = onClickNode;
    }

    /**
     * Adjusts the vertex node's position slightly to resolve positioning issues in split scenarios,
     * ensuring it stays within its parent's bounds.
     *
     * @author vittoriopiotti
     */
    public void forceMoveToFixBugOnSplit() {
        double newX = getCenterX() + 0.5;
        double newY = getCenterY() + 0.5;
        double width = getParent().getLayoutBounds().getWidth();
        double height = getParent().getLayoutBounds().getHeight();
        newX = boundVertexNodeXPositioning(newX, 0, width);
        newY = boundVertexNodeYPositioning(newY, 0, height);
    ...
    private void enableDrag() {
        ...
        setOnMousePressed((MouseEvent mouseEvent) -> {
                nodeStartX = mouseEvent.getSceneX();
                nodeStartY = mouseEvent.getSceneY();
        ...
        setOnMouseClicked((MouseEvent mouseEvent)->{
            if (!isDraggingNode) {
                onClickNode.accept(mouseEvent,this.getUnderlyingVertex());
            }
            mouseEvent.consume();
        });
        ...
        setOnMouseDragged((MouseEvent mouseEvent) -> {
            if (mouseEvent.isPrimaryButtonDown()) {
                if (Math.abs(mouseEvent.getSceneX() - nodeStartX) > dragThresholdNode ||
                        Math.abs(mouseEvent.getSceneY() - nodeStartY) > dragThresholdNode) {
                    isDraggingNode = true;
                    hideContextMenu.run();
                }
