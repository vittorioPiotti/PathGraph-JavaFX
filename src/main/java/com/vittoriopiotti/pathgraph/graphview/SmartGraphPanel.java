...
public class SmartGraphPanel<V, E> extends Pane {
    ...
    private final BiConsumer<MouseEvent, Edge<E, V>> _onClickEdge = (mouseEvent, edge) -> {
        ...
        if(isDoubleEdge(edge)){
            end = dir == Constants.DIRECTION_SECOND ? v1 : v2;
            start = dir == Constants.DIRECTION_SECOND  ? v2 : v1;
        }
     ...
     *
     * @param closeContextMenu a Runnable that defines the logic for hiding the context menu.
     */
    ...
    private Edge<E,V> _getEdge(char start,char end){
        for(Edge<E,V> e : edgeNodes.keySet()){
            ...
            if(isDoubleEdge(e)){
                if(e.getDirection() == Constants.DIRECTION_FIRST){
     ...
     * @author vittoriopiotti
     */
    public boolean setCost(char start, char end, int edgeLabel) {
        if(isDoubleEdge(start,end)){
            try{
                Edge<E,V> e = _getEdge(start,end);
                if(e != null){
                    return setCost(e,edgeLabel);
                }
            }catch(Exception ignored){
                return false;
            }
        }else{
            return setCost(getSpecificEdge(start,end),edgeLabel);
        }
        return false;
    }
    ...
    public boolean splitEdge(Edge<E, V> edge){
        ...
        if(!isDoubleEdge(edge)){
            ...
            try{
                ...
                Edge <E,V> e1 = getModel().insertEdge( outbound, inbound,  (E)generateIdEdge(),cost, Constants.DIRECTION_FIRST);
                getModel().insertEdge( inbound, outbound,  (E)generateIdEdge(),cost, Constants.DIRECTION_FIRST);
                updateViewModel();
    ...
    private boolean  rotateEdge( Edge<E, V>edge){
        if(isExistEdge(edge)) {
            ...
            if (edgeNode instanceof SmartGraphEdgeLine<E, V>) {
                final int NEW_DIRECTION = edgeNode.getDirection() == Constants.DIRECTION_FIRST ?
                        Constants.DIRECTION_SECOND
                        : edgeNode.getDirection() == Constants.DIRECTION_SECOND ?
                        Constants.BIDIRECTIONAL :
                        Constants.DIRECTION_FIRST;
                rotateEdge(edgeNode, NEW_DIRECTION, edgeNode.getDirection() != Constants.BIDIRECTIONAL);
                updateViewModel();
            } else {
                final int NEW_DIRECTION_FIRST = edgeNode.getDirection() == Constants.DIRECTION_FIRST ?
                        Constants.DIRECTION_SECOND :
                        Constants.DIRECTION_FIRST;
        ...
        try {
            ...
            if (direction != edgeNodes.get(e).getDirection()) {
                return switch (edgeNodes.get(e).getDirection()) {
                    case Constants.DIRECTION_FIRST -> {
                        if (direction == Constants.DIRECTION_SECOND) {
                            rotateEdge(v1, v2);
                        } else {
                            rotateEdge(v1, v2);
                            rotateEdge(v1, v2);
                    ...
                    case Constants.DIRECTION_SECOND -> {
                        if (direction == Constants.DIRECTION_FIRST) {
                            rotateEdge(v1, v2);
                            rotateEdge(v1, v2);
                        } else {
                            rotateEdge(v1, v2);
        ...
        if(isExistEdge(edge)) {
            ...
            if (isDoubleEdge(edge)) {
                try{
                    ...
                    removeEdge(edgeBase);
                    getModel().removeEdge(edge);
                    getModel().insertEdge(direction == Constants.DIRECTION_FIRST ? inbound : outbound, direction == Constants.DIRECTION_FIRST ? outbound : inbound, (E) generateIdEdge(), cost, direction);
                    updateViewModel();
                    return true;
                }catch(Exception ignored){
                }
    ...
    public boolean newEdge(char v1, char v2,int cost,int direction) {
        return newEdge(direction == Constants.DIRECTION_SECOND ? v2 : v1, direction == Constants.DIRECTION_SECOND ? v1 : v2, cost, direction != Constants.BIDIRECTIONAL) && rotateEdge(v1, v2, direction);
    }
    ...
    public int getDirectionRotated(int dir){
        return dir == Constants.DIRECTION_FIRST ? Constants.DIRECTION_SECOND : dir == Constants.DIRECTION_SECOND ? Constants.DIRECTION_FIRST : Constants.BIDIRECTIONAL;
    }
                ...
                for (int i = 1; i < lpn.size(); i++) {
                    ...
                    if (isExistNode(n) && isExistNode(_n)) {
                        ...
                        for (Edge<E, V> e : edgeNodes.keySet()) {
                                ...
                                if (isDoubleEdge(e)) {
                                    if (edgeNodes.get(e).getDirection() == Constants.DIRECTION_FIRST && isEqualEdge(edgeNodes.get(e), _vn, vn)) {
                                        edgeNodes.get(e).resetStyle();
                                    } else if (edgeNodes.get(e).getDirection() == Constants.DIRECTION_SECOND && isEqualEdge(edgeNodes.get(e), _vn, vn)) {
    ...
    public GraphDTO getGraph() {
        ...
        for (Edge<E,V> e : edgeNodes.keySet()) {
            ...
            edges.add(new EdgeDTO(
                    isDoubleEdge(e) ? i : d == Constants.DIRECTION_SECOND ? i : o,
                    isDoubleEdge(e) ? o : d == Constants.DIRECTION_SECOND ? o : i,
                    l,
                    d
            ));
        }
    ...
    public boolean isDoubleEdge(char start, char end) {
        return !(edgeNodes.get(getSpecificEdge(start, end)) instanceof SmartGraphEdgeLine);
    }
    ...
    private Edge<E,V> getSpecificEdge(char v1,char v2){
        try {
            ...
            for (Edge<E, V> e : edgeNodes.keySet()) {
                if(isDoubleEdge(e)){
                    if(isEqualEdge(edgeNodes.get(e),vb2,vb1) && edgeNodes.get(e).getDirection() == Constants.DIRECTION_FIRST ) {
                        return e;
    ...
    private boolean isEqualEdge(SmartGraphEdgeBase <E,V> e, char v1, char v2  ) {
        return e.getInbound().getAttachedLabel().getText().charAt(0) == v1 && e.getOutbound().getAttachedLabel().getText().charAt(0) == v2;
    }
    ...
    private Edge <E,V> highLevelStaticEdgeCreation(Vertex<V> v1,Vertex<V> v2,int cost,boolean isArrowed){
        Edge <E,V> e = this.getModel().insertEdge(
                ...
                cost,                                   //edge label
                isArrowed ? Constants.DIRECTION_FIRST  :  Constants.BIDIRECTIONAL     //edge direction: DIRECTION_FIRST, DIRECTION_SECOND, BIDIRECTIONAL
        );
        updateViewModel();
        return e;
    }
     ...
     * @author vittoriopiotti
     */
    public boolean isActiveVertex(char c){
        try {
            Vertex<V> v = getNode(c);
            if(v != null){
                return isActiveVertex(v);
            }
            return false;
        }catch(Exception ignored){
            return false;
     ...
     * @author vittoriopiotti
     */
    private boolean isActiveVertex(Vertex<V> v){
     ...
     * other vertices connected to the specified vertex.
     *
     * @param c the vertex label to disable connections for
     * @return true if the connections were successfully disabled, false if the vertex does not exist
     *
     * @author vittoriopiotti
     */
    public boolean disabledConnectionsWithNode(char c){
    ...
    public SmartStylableNode getStylableEdge(char start,char end) {
     ...
     */
    public SmartStylableNode getStylableVertex(char v){
