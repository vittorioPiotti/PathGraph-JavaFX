...
import com.vittoriopiotti.pathgraph.callbacks.GenericCallback;
...
public class UtilitiesUI {
    ...
    public static Slider createSlider(Consumer<Double> setZoom,double initialZoom){
        ...
        slider.valueProperty().addListener((observable, oldValue, newValue) -> setZoom.accept((double)newValue));
        return slider;
    }
    /**
    ...
    public static Pane createJoystick(BiConsumer<Double, Double> callback) {
        ...
        Timeline updateStateTimeline = getTimeline(callback, joystick, handle);
    ...
    /**
     * Creates a {@link Timeline} to update joystick handle position.
     *
     * @param callback the action to perform with direction values.
     * @param joystick the joystick circle.
     * @param handle   the handle circle.
     * @return a Timeline for joystick updates.
     */
    private static Timeline getTimeline(BiConsumer<Double, Double> callback, Circle joystick, Circle handle) {
     ...
     * @param maxLength    the maximum length of text that can be entered in the field.
     * @param width        the width of the text field.
     * @param eventHandler an {@link GenericCallback}
     * @param params       additional parameters passed to the event handle
     ...
     *
     * @throws IllegalArgumentException if the maximum length is less than zero, or if the {@code eventHandler} is of type {@link Object}
     *                                  and {@code params} is not an array of characters with at least two elements.
     */
    public static TextField createTextField(String promptText, boolean numbers, boolean strings, boolean focus,
                                            int maxLength, double width,
                                            double v1, double v2, double v3, double v4,
                                            double b1, double b2, double b3, double b4 ,Object eventHandler, Object... params) {
        ...
        textField.setOnKeyPressed(event -> {
            ...
            if (event.getCode() == KeyCode.ENTER) {
                if (eventHandler instanceof GenericCallback) {
                    ((GenericCallback) eventHandler).handle(
                            Stream.concat(Stream.of((Object) null), Arrays.stream(params)).toArray(Object[]::new)
                    );
                    ...
                    biConsumer.accept(null, params.length > 0 ? params[0] : null);
                }else if (eventHandler instanceof Runnable) {
        ...
        if (focus) {
            ...
            Platform.runLater(()->{
                if (textField.getParent() != null) {
                    ...
                    textField.getScene().getWindow().requestFocus();
                }
            });
        }
     ...
     * @param eventHandler an event handler type of {@link GenericCallback}
     * @param params additional parameters to be passed to the event handler.
     *
     ...
     */
    public static Button createButton(String svgContent1, String svgContent2, Color color, double width,
                                      double v, double v1, double v2, double v3,
                                      double b, double b1, double b2, double b3,
                                      Object eventHandler, Object... params) {
        ...
        button.setPrefHeight(svgContent2 == null || svgContent2.isEmpty() ? 25 : 50);
        ...
        button.setOnMouseEntered((e) -> button.setBackground(new Background(new BackgroundFill(Color.web("#d3d4d5"),
                new CornerRadii(v, v1, v2, v3, false), Insets.EMPTY))));

        button.setOnMouseExited((e) -> button.setBackground(new Background(new BackgroundFill(Color.web("#f8f9fa"),
                new CornerRadii(v, v1, v2, v3, false), Insets.EMPTY))));

        if (eventHandler instanceof GenericCallback) {
            button.setOnMouseClicked(mouseEvent ->
                    ((GenericCallback) eventHandler).handle(
                            Stream.concat(Stream.of(mouseEvent), Arrays.stream(params)).toArray(Object[]::new)
                    )
            );
        } else if (eventHandler instanceof BiConsumer) {
            @SuppressWarnings("unchecked")
            BiConsumer<MouseEvent, Object> biConsumer = (BiConsumer<MouseEvent, Object>) eventHandler;
            button.setOnMouseClicked((mouseEvent) -> biConsumer.accept(mouseEvent, params.length > 0 ? params[0] : null));
        } else if (eventHandler instanceof Runnable) {
            button.setOnMouseClicked((MouseEvent mouseEvent) -> ((Runnable) eventHandler).run());
        }
