...
public class EdgeDTO {
    ...
    public EdgeDTO(char from, char to, int cost, int dir) {
        if (dir == Constants.OPPOSITE_DIRECTION) {
            this.from = to;
            this.to = from;
        } else {
            this.from = from;
        ...
        this.cost = cost;
        this.isArrowed = dir != Constants.BIDIRECTIONAL;
    }
     ...
     *
     * @return a string in the format "EdgeDTO{from=X, to=Y, cost=Z, isArrowed=W}"
     *         where X is the starting point, Y is the endpoint, Z is the cost, and W indicates if the edge is directed.
     */
    @Override
