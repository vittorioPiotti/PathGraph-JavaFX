    ...
    private static CompletableFuture<Integer> takeSnapshot(Pane parent, double scaleFactor) {
        ...
        parent.snapshot(snapshotResult -> {
            ...
            if (snapshot.getWidth() == 0 || snapshot.getHeight() == 0) {
                snapshotFuture.complete(AppConstants.ERROR);
            } else {
        ...
        Platform.runLater(() -> {
            ...
            if (file != null) {
                try {
                    ...
                    saveFuture.complete(AppConstants.SUCCESS);
                } catch (IOException ignored) {
                    saveFuture.complete(AppConstants.ERROR);
                }
            } else {
                saveFuture.complete(AppConstants.INTERRUPTED);
            }
        });
